...
# Performs actual creation of new CustomersController
  ...
  def result
    ...
    begin
      CSV.foreach(tmpfile.path, {:headers=>true, :encoding=>encoding, :quote_char=> quote_chars.shift, :col_sep=>splitter}) do |row|
        customer = Customer.find_by_email(row[attrs_map["email"]])
        unless customer
          customer = Customer.new()
        else
          ...
          customer.visible_custom_field_values.each do |custom_field_value|
            if custom_field_value.custom_field.name == attrs_map[custom_field_value.custom_field.name]
              custom_field_value.value = row[attrs_map[custom_field_value.custom_field.name]]
            end
          end
        end
...
# Internal method for setting global customer
  ...
  def set_customer
    if params[:id] != "bobo"
      @customer = Customer.where(active: true).find(params[:id])
    end
  rescue ActiveRecord::RecordNotFound
    render_404
  end
